...
export const QuizListPage = () => {
  ...
  const [pagination, setPagination] = useState<PaginationState>({
    ...
    const { data: quizList } = useGetMaterialList(
  ...
  return (
    <div className="relative overflow-y-hidden">
      <div
        className="flex flex-row w-[90dvw] md:w-[97dvw] items-center justify-between max-h-screen bg-white/70 border-solid rounded-lg p-3 shadow-[0px_2px_4px_-2px_rgba(16,24,40,0.06),0px_4px_8px_-2px_rgba(16,24,40,0.10)] translate-y-3 mx-3"
      >
        <div className="flex flex-col w-full mt-3 ml-3">
          <div className="w-full">
            <p className="text-[20px] font-semibold">Materi Kuis</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 max-h-[65dvh] overflow-y-auto mb-5">
                        ...
                        <div className="relative border-solid rounded-lg p-3 bg-white mt-5 mx-3 shadow-md">
                            <div className="flex flex-row items-center mx-3 gap-2 mb-2">
                                <div className={`border-solid rounded-lg p-3 flex flex-row gap-2 ${item.difficulty === "Novice" ? "bg-[#5A86A0]" : item.difficulty === "Hard" ? "bg-[#00639D]" : "bg-[#2F81B1]"} w-full h-[100px]`}>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[11px] font-semibold text-white">Grade: {GradeText[item.grade as keyof typeof GradeText]}</p>
                                        <p className="text-[11px] font-semibold text-white">Difficulty: {item.difficulty}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="w-full flex justify-end pr-3">
                                <Button
                                    ...
                                    onClick={() => navigate(`/quiz/${item.materialName}`)}
                                >
                                    Lihat Kuis
                                </Button>
                            </div>
